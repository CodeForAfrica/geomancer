        ...
        for column in columns:
            ...
            for geo_type, geo_id in geo_ids:
                if not results.get(geo_id):
                    results[geo_id] = []
                results[geo_id].append((item for item in DATA if item.keys()[0] == geo_id).next().values()[0][COLUMNS.index(column)])
        return results
